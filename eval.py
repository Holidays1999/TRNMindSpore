...
from mindspore import Model, nn, load_checkpoint, load_param_into_net, context, set_seed
...
def initialize_trn_network(trn_net, checkpoint_path):
    ...
    if os.path.isfile(checkpoint_path):
        ...
        print(f"No pretrained checkpoint is found at {checkpoint_path}")
...
def run_eval(cfg):
    ...
    if os.path.isdir(cfg.ckpt_file):
        ckpt_files = glob.glob(os.path.join(cfg.ckpt_file, "*.ckpt"))
        for ckpt_file in ckpt_files:
            cfg.ckpt_file = ckpt_file
            logger.info('load_checkpoint: %s', cfg.ckpt_file)
            initialize_trn_network(network, cfg.ckpt_file)
            logger.info('Checkpoint loaded!')
